...
@foreach ($data as $index => $monthData)
    ...
    @php
        ...
        $gabunganPemasukkan = [];
        $pemasukkanRows = is_array($monthData['pemasukkan'])
            ? $monthData['pemasukkan']
            : $monthData['pemasukkan']->toArray();
        $pemasukanDendaRows = is_array($monthData['pemasukan_denda'])
            ? $monthData['pemasukan_denda']
            : $monthData['pemasukan_denda']->toArray();
        $pengeluaranRows = is_array($monthData['pengeluaran'])
            ? $monthData['pengeluaran']
            : $monthData['pengeluaran']->toArray();

        foreach ($pemasukkanRows as $p) {
            $gabunganPemasukkan[] = array_merge($p, ['sumber' => 'Pembayaran']);
        }

        foreach ($pemasukanDendaRows as $d) {
            $gabunganPemasukkan[] = array_merge($d, ['sumber' => 'Denda']);
        }

        usort($gabunganPemasukkan, function ($a, $b) {
            return strcmp($a['tanggal_sort'] ?? '', $b['tanggal_sort'] ?? '');
        });

        $pengeluaranSorted = $pengeluaranRows;
        usort($pengeluaranSorted, function ($a, $b) {
            return strcmp($a['tanggal_sort'] ?? '', $b['tanggal_sort'] ?? '');
        });

        $max = max(count($gabunganPemasukkan), count($pengeluaranSorted));
    @endphp
    ...
    <table style="width: 100%;">
        <tr>
            ...
            <td style="text-align: right;border-color: #FFFFFF;">{{ number_format($gabunganPemasukkan[$i]['jumlah'] ?? 0, 0, ',', '.') }}</td>
        </tr>
    </table>
</td>
    ...
    {{ $gabunganPemasukkan[$i]['keterangan'] ?? '-' }}
                ...
                <td style=" text-align: center;" >{{ $pengeluaranSorted[$i]['tanggal'] ?? '' }}</td>
                <td  style="text-align: start;width: 150px;"> {{ $pengeluaranSorted[$i]['jenis'] ?? '' }}</td>
   <td style="width: 80px; padding:2px;">
    <table style="width: 100%;">
        <tr>
            <td style="text-align: left; border-color: #FFFFFF;">Rp</td>
            <td style="text-align: right;border-color: #FFFFFF;">{{ number_format($pengeluaranSorted[$i]['jumlah'] ?? 0, 0, ',', '.') }}</td>
        </tr>
    </table>
</td>
            <td style="width: 90px; text-align: center;">
    {{ $pengeluaranSorted[$i]['keterangan'] ?? '-' }}
</td>
            ...
            @php
                ...
                $totalPengeluaran += $pengeluaranSorted[$i]['jumlah'] ?? 0;
            @endphp
        @endfor
